...
class Args(argparse.Namespace):
    ...
    thirty2bit: bool
    no_xdg: bool
    windows_version: Sequence[Literal['10', 'vista', '2k3', '7', '8', 'xp',
                                      '81', '2k', '98', '95']]
    sandbox: bool
    vd: str
    verbose: bool
...
def run(args: Any, **kwargs: Any) -> sp.CompletedProcess:
...
def wine(args: Args) -> int:
    ...
    if isdir(target):
        ...
        return 1
    arch: Final[Literal['win32',
                        'win64']] = ('win32' if args.thirty2bit else 'win64')
    ...
    if args.no_xdg:
        try:
            run(('wine', 'reg', 'add', r'HKCU\Software\Wine\DllOverrides',
    ...
    target = quote(target)
    try:
        winetricks: Optional[str] = run(('sh', '-c', 'command -v winetricks'),
                                        stdout=sp.PIPE,
                                        stderr=sp.PIPE,
                                        check=True,
    ...
    winetricks_args = [
        winetricks, prefix_arg, VERSION_MAPPING[args.windows_version[0]]
    ]
    if args.sandbox:
