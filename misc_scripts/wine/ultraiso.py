...
class ExistantFile(argparse.Action):
    def __call__(self,
                 ...
                 _option_string: Optional[str] = None) -> None:
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description='UltraISO command line access.')
    ...
    if argv_len == 1 or (argv_len == 2 and isfile(sys.argv[1])):
        ...
        proc = sp.run(  # pylint: disable=subprocess-run-check
            ['wine', actual_exe_path] + sys.argv[1:],
    ...
    info_group.add_argument('-volset',
                            help='Volume set ID',
                            dest='volset',
                            type=int)
    ...
    fs_group.add_argument('-rockridge',
                          ...
                          action='store_true',
                          dest='rockridge')
    fs_group.add_argument('-udf',
                          help='Create UDF volume',
                          action='store_true',
                          dest='udf')
    fs_group.add_argument('-hfs',
                          help='Create Apple HFS volume',
                          action='store_true',
                          dest='hfs')
    ...
    boot_group.add_argument('-bootfile',
                            help='Set boot file',
                            action=ExistantFile,
                            dest='bootfile')
    ...
    add_file_or_dir_group.add_argument(
        '--file',
        dest='file',
        metavar='filename',
        action=ExistantFile,
    ...
    ultraiso_only.add_argument('-password',
                               metavar='PASSWORD',
                               help='Set password')
    ...
    info_only.add_argument('-list',
                           help='Create a list of files and directores in an'
                           ' ISO image')
    info_only.add_argument('-get',
                           help='Set a file or directory (full path) to be'
                           ' extracted')
    ...
    for arg in vars(args):
        ...
        if arg in need_two_dashes and arg not in fix_to_one:
            sp_args.append('--%s' % (arg, ))
        else:
            sp_args.append('-%s' % (arg, ))
    ...
    wine_prefix_env = quote(f'WINEPREFIX={args.prefix}')
    log.debug('env %s %s', wine_prefix_env,
              ' '.join(quote(x) for x in sp_args))
    p = sp.run(  # pylint: disable=subprocess-run-check
        sp_args,
        stderr=sp.PIPE,
        env=env,
        text=True)
