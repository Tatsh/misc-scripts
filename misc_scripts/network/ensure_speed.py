...
syslog_handle: Optional[SysLogHandler] = None


def speedtest(*args: str, **kwargs: Any) -> sp.CompletedProcess:
    kwargs.pop('check')
    kwargs.pop('stdout')
    kwargs.pop('encoding')
    kwargs['check'] = True
...
def maybe_log_yaml(handle: Optional[TextIO], **kwargs: Any) -> None:
    if handle:
...
def main() -> int:
    ...
    log_yaml_handle: Optional[TextIO] = None
    failed = False
    if args.log_file:
        ...
        if HAS_YAML:
            log_yaml_handle = open(args.log_file + '.yml', 'a+')
    ...
    maybe_log_yaml(log_yaml_handle,
                   ping=ping,
                   download=dl,
                   upload=ul,
                   share_uri=share)
