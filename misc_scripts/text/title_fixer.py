...
from typing import Iterable, Mapping, Match, Optional, Sequence, cast
import argparse
import re
import sys
...
JAPANESE_PARTICLES: Final[Sequence[str]] = ('de', 'e', 'ga', 'ha', 'ka',
                                            'kana', 'ne', 'ni', 'no', 'to',
                                            'wa', 'wo')
CHINESE_PARTICLES: Final[Sequence[str]] = ('de', 'ge', 'he', 'le', 'ma')
# NOTE This list is not yet complete
ARABIC_STOPS: Final[Sequence[str]] = ('al', 'ala', 'alayhi', 'alayka',
                                      'alayya', 'an', 'anhu', 'anka', 'anni',
                                      'bi', 'biha', 'bihi', 'bika', 'fi',
                                      'fihi', 'fika', 'fiya', 'ila', 'ilayhi',
                                      'ilayka', 'ilayya', 'lahu', 'laka', 'li',
                                      'maa', 'maahu', 'maaka', 'mai', 'min',
                                      'minhu', 'minka', 'minni', 'wa')
...
def get_name(word: str) -> Optional[str]:
    word = word.lower()
    ...
    for mode in modes:
        ...
        for word in word_list[1:]:
            ...
            if re.match(r'\w+\W$', word):
                end = cast(Match[str], re.match(r'\w+(\W+)$',
                                                word)).groups()[0]
                word = word[0:-1]
