...
def _parse_args() -> MainArgs:
    ...
    if IS_WINDOWS:
        parser.add_argument('--window-style',
                            type=int,
                            default=1,
                            help='Window style (int)')
...
def _get_icon_from_site(args: MainArgs,
    ...
    log = setup_logging_stderr('_get_icon_from_site', args.verbose)
    if args.icon:
        return args.icon
    ...
    sizes = cast(List[Tuple[int, str]],
                 [(0, urljoin(args.url, '/favicon.ico'))])
        ...
        if uri:
            if 'sizes' in tag.attrs:
                for width, height in map(lambda x: map(int, x.split('x')),
                    ...
                    log.debug('Icon with size %dx%d at URI %s', width, height,
                              icon_uri)
                    sizes.append(
                        (width if width > height else height, icon_uri))
            else:
                icon_uri = urljoin(args.url, uri)
...
def _ico_index(tmp_name: str, verbose: Optional[bool] = False) -> str:
    log = setup_logging_stderr(name='_ico_index', verbose=verbose)
...
def _fetch_convert_icon(  # pylint: disable=too-many-branches,too-many-statements
        ...
        verbose: Optional[bool] = False) -> Optional[str]:
    log = setup_logging_stderr(name='_fetch_convert_icon', verbose=verbose)
...
def _create_desktop_file(args: MainArgs) -> None:
    log = setup_logging_stderr(name='_create_desktop_file',
...
''')
    ...
    subprocess.check_call(
        ('xdg-desktop-menu', 'install', chrome_apps_dir_file, out_name),
...
def _create_app_bundle(args: MainArgs, short_name: str) -> None:
    ...
    bundle_id = ('com.google.Chrome.app.' f'{profile_dir_short}-{short_name}')
    ...
    with open(script, 'w+') as f:
        ...
        f.write(f"open -n '{args.chrome_command}' --args "
                f"'--app={args.url}' "
                f'--user-data-directory={args.profile_name}\n')
    chmod(script, 0o755)
...
def _create_shortcut(args: MainArgs) -> int:
    log = setup_logging_stderr(name='_create_shortcut', verbose=args.verbose)
...
def _make_short_name(url: str, name: str) -> str:
    parsed = urlparse(url)
    split_netloc = list(
        reversed(re.sub(r'^www\.', '', parsed.netloc).split('.')))
    if len(split_netloc) == 2:
        if parsed.path and parsed.path != '/':
            ...
            split_netloc += [
                re.sub(r'\s+', '-', name.strip()).lower()[:64].rstrip('.')
            ]
...
def main() -> int:
    ...
    if not args.icon and IS_MAC:
        ...
        return 1
    args.icon = _fetch_convert_icon(args.icon, short_name, session,
                                    args.verbose)
    if IS_LINUX:
