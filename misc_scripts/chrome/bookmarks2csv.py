...
from typing import (Any, Iterable, Iterator, Optional, Sequence, TextIO, Tuple,
                    Union, cast)
import argparse
import csv
import json
...
def get_strings(children: Iterable[Union[URL, Folder]],
                folder_names: bool = True,
                ignore_empty_names: bool = False,
                prefix: Optional[str] = None) -> Iterator[StringTuple]:
    for x in children:
        if x['type'] == 'folder':
            name = x['name']
            yield from get_strings(
                cast(Folder, x)['children'],  # type: ignore[redundant-cast]
            ...
            yield (chrome_timestamp_to_date(
                x['date_added']).strftime('%Y-%m-%d'), last_visited, name,
                   cast(URL, x)['url'])  # type: ignore[redundant-cast]
...
def main() -> int:
    ...
    try:
        ...
        for root in ('bookmark_bar', 'other'):
            _ = data['roots'][cast(RootKey, root)]['children']
